...
namespace WBS.Net
{
    public class WBS
    {
        ...
        public async void Valid(string verifier, int userId)
        {
            ...
            //string responseText0 = session.Request(accessToken).Get().ForUrl(string.Format("http://wbsapi.withings.net/user?action=update&userid={0}", userId)).ToString();
            ...
            //string responseText2 = session.Request(accessToken).Get().ForUrl(string.Format("http://wbsapi.withings.net/measure?action=getmeas&userid={0}", userId)).ToString();
        }
        ...
        public async Task<List<MeasureGroup>> GetMeasures(DateTime startDate, DateTime endDate)
        {
            return await Task.Run<List<MeasureGroup>>(() =>
            {
                ...
                string responseText = session.Request(accessToken).Get().ForUrl(string.Format("http://wbsapi.withings.net/measure?action=getmeas&userid={0}&startdate={1}&enddate={2}", userId, ToUnixTime(startDate), ToUnixTime(endDate))).ToString();
